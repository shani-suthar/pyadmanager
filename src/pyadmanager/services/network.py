...
class NetworkClient:
    ...
    with no documented filterable fields — `list` simply returns
    ...
    def list(self, page_size: int = 1000):
        ...
        `list` methods there's no `filter` query param to build.
        """
        params = {"pageSize": page_size}
        return self.http_client.fetch_all(self._gam_obj_type, self._gam_obj_type, params)

    def get(self, network_code: str | int | None = None):
        ...
        via `list`). Builds the path directly rather than via
