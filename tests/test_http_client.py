...
class TestFetch:
    def test_returns_parsed_json(self, http_client):
        http_client.authed_session.get.return_value = make_response({"foo": "bar"})
    ...
    def test_raises_on_http_error(self, http_client):
        ...
        http_client.authed_session.get.return_value = response
...
class TestFetchAll:
    def test_collects_gam_obj_type_across_pages(self, http_client):
        http_client.authed_session.get.side_effect = [
        ...
        assert http_client.authed_session.get.call_count == 2
    ...
    def test_follows_next_page_token_in_params(self, http_client):
        ...
        second_call_params = http_client.authed_session.get.call_args_list[1].kwargs["params"]
        assert second_call_params["pageToken"] == "abc"

    def test_stops_when_no_next_page_token(self, http_client):
        http_client.authed_session.get.return_value = make_response({"lineItems": [1, 2]})

        results = http_client.fetch_all("networks/1/lineItems", "lineItems")

        assert results == [1, 2]
        assert http_client.authed_session.get.call_count == 1

    def test_missing_gam_obj_type_key_yields_empty_page(self, http_client):
        http_client.authed_session.get.return_value = make_response({})
